...
class UpdateChannelScreen extends StatefulWidget {
  ...
  const UpdateChannelScreen({Key? key, required this.channel}) : super(key: key);
...
class _UpdateChannelScreenState extends State<UpdateChannelScreen> {
  ...
  Widget build(BuildContext context) {
    _descriptionController = TextEditingController(text: widget.channel.description);
     _channelNameController = TextEditingController(text: widget.channel.channel);
            ...
            child: Column(
                children:[
                  ...
                  Row(
                    ...
                    children: [
                      MainButton(label: "Update Channel", onTap: ()=>_validateFormData())
                    ],
                  )
                ]
            )
  ...
  _validateFormData(){
    if(_channelNameController.text.isNotEmpty && _descriptionController.text.isNotEmpty && _imageController.text.isNotEmpty){

      _saveFormDataToDB();
      Get.back();

    }else if(
    _channelNameController.text.isEmpty || _descriptionController.text.isEmpty || _imageController.text.isEmpty
    ) {
